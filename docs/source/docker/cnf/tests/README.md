...
Zuul Check for testing `Injection and Rotation of External CA on CAPI Clusters` is being handled by the job -  `airship-airshipctl-gate-script-runner-cert_managament_test`, included in the patchset https://review.opendev.org/#/c/739034/7

For more information on Injection and Rotation of External CA on CAPI Clusters visit docs/source/certificate_management/README.md on https://review.opendev.org/#/c/739034/7

## Scripts And Usage

| script name                                                 | purpose                                                                                                                                                                                                            |
| ----------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| tools/deployment/docker/52_cluster_init_target_node.sh      | - generate docker provider metadata <br> - initialize kind cluster with cluster api and cluster api docker provider components <br> - test if control plane is up                                                  |
| tools/deployment/docker/53_cluster_move_to_target_node.sh   | - move cluster metadata to target cluster <br> - make target cluster self manageD                                                                                                                                  |
| tools/deployment/docker/54_deploy_CNF_n_deps_target_node.sh | - install helm operator (helm-controller) <br> - install multus cni plugin (dependency for cnf) <br> - install the sample CNF (VPP ipforwarder)                                                                    |
...
`$ ./tools/deployment/docker/52_cluster_init_target_node.sh`

```
TODO...
```

`$ ./tools/deployment/docker/53_cluster_move_to_target_node.sh`

```
TODO...
```

`$ ./tools/deployment/docker/54_deploy_CNF_n_depends_target_node.sh`
